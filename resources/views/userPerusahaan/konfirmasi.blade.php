...
<body>
                ...
                <div class="row">
                    ...
                    <div class="col-sm-12 col-md-12 col-lg-12 col-xl-6">
                        <div class="contact-panel">
                            <form action="/postKonfirmasi/{{ $lamarr->id }}" method="POST"
                                ...
                                <div class="row">
                                    ...
                                    <div class="col-12">
                                        ...
                                        <div class="form-check form-check-inline">
                                            <input class="form-check-input" type="radio" name="status" value="Pemanggilan interview"
                                                @if ($lamarr->status == 'Pemanggilan Interview') checked @endif onclick="toggleFileInput(false)">
                                            <label class="form-check-label" for="inlineRadio1">Pemanggilan interview</label>
                                        </div>
                                        <div class="form-check form-check-inline">
                                            <input class="form-check-input" type="radio" name="status" value="Lamaran diterima"
                                                @if ($lamarr->status === 'Lamaran diterima') checked @endif onclick="toggleFileInput(true)">
                                            <label class="form-check-label" for="inlineRadio2">Lamaran diterima</label>
                                        </div>
                                        <div class="form-check form-check-inline">
                                            <input class="form-check-input" type="radio" name="status" value="Lamaran Ditolak"
                                                @if ($lamarr->status == 'Lamaran ditolak') checked @endif onclick="toggleFileInput(false)">
                                            <label class="form-check-label" for="inlineRadio3">Lamaran Ditolak</label>
                                        </div>
                                        
                                        <div class="col-12 mt-3" id="fileInputContainer" style="display: none;">
                                            <div class="form-group">
                                                <label for="ijazahInput">Upload Surat Pemanggilan</label>
                                                <input type="file" id="ijazahInput" name="suratpemanggilan" class="form-control-file">
                                                @if ($lamarr->suratpemanggilan)
                                               <div class="file-name">File terpilih: {{ $lamarr->suratpemanggilan }}</div>
                                               @endif
                                               <span class="text-danger">
                                                   @error('suratpemanggilan')
                                                   {{ $message }}
                                                   @enderror
                                               </span>
    ...
    <script>
       function toggleFileInput(showFileInput) {
        var fileInputContainer = document.getElementById('fileInputContainer');
        fileInputContainer.style.display = showFileInput ? 'block' : 'none';
    }
