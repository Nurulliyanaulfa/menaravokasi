...
<body>
  ...
  <section class="main">
    <div class="main-top">
        <form action="{{ route('userPerusahaan.pelamar', ['posting' => $posting->id]) }}" method="GET">
            @csrf
        </form>
    </div>
    <div class="users">
            ...
            <div class="table-responsive" style="overflow-x: auto;">
                <table class="table" style="margin-top: 10px">
                    ...
                    <tbody class="table-group-divider">
                        @foreach ($pelamar as $index => $pelamar)
                        <tr>
                            <td>{{ optional($pelamar->user)->name }}</td>
                            <td>
                                <a href="{{ asset('storage/' . $pelamar->ijazah) }}" target="_blank">Buka Ijazah</a>
                            </td>
                            <td>
                                <a href="{{ asset('storage/' . $pelamar->surat) }}" target="_blank">Buka Surat Lamaran</a>
                            </td>
                            <td><a href="{{ $pelamar->keterangan }}" target="_blank">Lihat persyaratan lainnya</a></td>
                            <td>
                                <span class="{{ $pelamar->status === 'sedang diproses' ? 'fw-semibold text-success' : 'fw-semibold text-danger' }}">
                                    {{ $pelamar->status }}
                                </span>
                            </td>
                            <td>
                                @if ($pelamar->status == 'Lamaran selesai')
                                <i class="far fa-check-circle text-success"></i>
                                @else
                                <a class="button" href="{{ route('konfirmasi', $pelamar->id) }}">Konfirmasi</a><br>
                                @endif
                            </td>
                        </tr>
                        @endforeach
