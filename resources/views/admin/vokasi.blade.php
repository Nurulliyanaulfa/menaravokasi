      ...
      <div class="users">
        <div class="card">
          <div class="row mt-5">
            ...
            <div class="col"></div>
            {{-- <div class="col-2">
              <a class="btn btn-success" href="{{ route('admin.tambahPerusahaan') }}" style="text-decoration: none; margin-left: 30px">Tambah Data +</a>
            </div> --}}
          </div>
                ...
                <tbody class="table-group-divider">
                  @foreach ($data as $user)
                      <tr>
                          ...
                          <td>
                            @foreach ($user->lamarrans as $lamarr)
                          ...
                          <td>
                            @foreach ($user->lamarrans as $lamarr)
                                @if ($lamarr->status === 'Lamaran diterima')
                                    <span class="fw-semibold {{ $lamarr->status === 'sedang diproses' ? 'text-success' : 'text-danger' }}">
                                        {{ $lamarr->status }}
                                    </span>
                                    <br>
                                    ...
                                    <a href="{{ asset('storage/' . $lamarr->pembayaran) }}" target="_blank" style="color: blue; text-decoration: underline;">Buka Bukti</a>
                        ...
                        <td>
                          @foreach ($user->lamarrans as $lamarr)
                              @if ($lamarr->status === 'Lamaran diterima')
                                  <form action="{{ route('konfirmasiPembayaran', ['id' => $lamarr->id]) }}" method="get">
                                      @csrf
                          ...
                          <td>
                              <form action="{{ route('admin.unblockUser', ['id' => $user->id]) }}">
                                  @csrf
                                  <button type="submit" class="btn btn-success">Buka Blokir</button>
                              </form>
              
                              <!-- Button untuk Blokir -->
                              <form action="{{ route('admin.blokirUser', ['id' => $user->id]) }}">
                                  @csrf
